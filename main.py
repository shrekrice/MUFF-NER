...
def data_initialization(data, gaz_file, train_file, dev_file, test_file):#建立语料库"gaz_file指的是../CNNNERmodel/data/ctb.50d.vec"，其他文件指对应文件
    ...
    data.build_gaz_alphabet(dev_file,count=True)#建立词典库
    data.build_gaz_alphabet(test_file,count=True)#word_list = []装入
    data.fix_alphabet()
    return data#data为Data类
...
def predict_check(pred_variable, gold_variable, mask_variable):
    ...
    return right_token, total_token#返回正确的token，和全部的total_token
...
def train(data, save_model_dir, seg=True):
    ...
    print("Training with {} model.".format(data.model_type))#lstm
    ...
    for idx in range(data.HP_iteration):
        ...
        print(("Epoch: %s/%s" %(idx,data.HP_iteration)))#显示epoch的轮数
