...
class Data:
    ...
    def build_alphabet(self, input_file):
        ...
        for idx in range(len(in_lines)):#长度共为704368
            ...
            if len(line) > 2:
                ...
                label = pairs[-1]#取每一行的最后一个为label
                self.label_alphabet.add(label)#添加label
                self.word_alphabet.add(word)#添加word

                if idx < len(in_lines) - 1 and len(in_lines[idx+1]) > 2:
                    biword = word + in_lines[idx+1].strip().split()[0]#将相邻的单词拼接在一起，形成biword
                else:
                    biword = word + NULLKEY
                self.biword_alphabet.add(biword)#向biword_alphabet中加入biword
                # biword_index = self.biword_alphabet.get_index(biword)
                self.biword_count[biword] = self.biword_count.get(biword,0) + 1#biword_count列表形式，为每个biword赋值为1，形式为{'word1':1}
                for char in word:
                    self.char_alphabet.add(char)#组成字符集

                seqlen += 1
            else:
                seqlen = 0

        self.word_alphabet_size = self.word_alphabet.size()#len(instances)+1
        self.biword_alphabet_size = self.biword_alphabet.size()
        self.char_alphabet_size = self.char_alphabet.size()
        self.label_alphabet_size = self.label_alphabet.size()#得到所有数据集的大小
        startS = False
        startB = False
        for label,_ in self.label_alphabet.iteritems():
            if "S-" in label.upper():
        ...
        if startB:
            if startS:
                ...
                self.tagScheme = "BIO"#如果存在s这一标签则为BMES标签，反之则为BIO标签
    ...
    def build_gaz_file(self, gaz_file):
        ...
        if gaz_file:
            ...
            for fin in fins:
                fin = fin.strip().split()[0]#去词典每一行的第一个词
                if fin:
                    self.gaz.insert(fin, "one_source")#fin为一个列表
            print ("Load gaz file: ", gaz_file, " total size:", self.gaz.size())
        else:
            print ("Gaz file is None, load nothing")


    def build_gaz_alphabet(self, input_file, count=False):#输入文件为训练、测试、验证
        in_lines = open(input_file,'r',encoding="utf-8").readlines()
        word_list = []
        for line in in_lines:
            if len(line) > 3:
...
if __name__ == '__main__':
    ...
    f=Data()
    print(f.build_gaz_alphabet(input_file))
