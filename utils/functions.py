    ...
    for idx in range(len(in_lines)):
        ...
        if len(line) > 2:
            ...
            if number_normalized:
                word = normalize_word(word)#如果存在number_normalized
            label = pairs[-1]
...
#返回的是instence_texts, instence_Ids，相互对应
...
def load_pretrain_emb(embedding_path):
    ...
    with open(embedding_path, 'r',encoding="utf-8") as file:
        for line in file:
            ...
            if len(line) == 0:
                continue
            tokens = line.split()#切割每个字符并存入一个列表中
            if embedd_dim < 0:
                embedd_dim = len(tokens) - 1#增加embedding的维度
            else:
                assert (embedd_dim + 1 == len(tokens))#断言函数，如判定为真则可继续运行
            embedd = np.empty([1, embedd_dim])#输出一个一行embedd_dim列的向量
            embedd[:] = tokens[1:]#embedd中所以[]与tokens[1:]相等。减少一个维度
            embedd_dict[tokens[0]] = embedd
    return embedd_dict, embedd_dim
#返回embedd_dict, embedd_dim，embedd_dict为产生的embedding，为不重复的字符
