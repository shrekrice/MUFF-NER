...
class Alphabet:
    def __init__(self, name, label=False, keep_growing=True):
        ...
        # Index 0 is occupied by default, all else following.索引 0 默认被占用，其他的都在后面。
    ...
    def clear(self, keep_growing=True):
        ...
        self.default_index = 0
        self.next_index = 1#重置instances
        
    def add(self, instance):
        if instance not in self.instance2index:
            self.instances.append(instance)
            self.instance2index[instance] = self.next_index
            self.next_index += 1#加入该instances
    ...
    def get_instance(self, index):
        if index == 0:
            # First index is occupied by the wildcard element.第一个索引被通配符元素占据。
            return None
        try:
