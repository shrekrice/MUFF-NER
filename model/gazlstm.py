...
class WPLSTM(nn.Module):
    def __init__(self, data):
        ...
        scale = np.sqrt(3.0 / self.gaz_emb_dim)#scale
        data.pretrain_gaz_embedding[0,:] = np.random.uniform(-scale, scale, [1, self.gaz_emb_dim])#X[n,:]是取第1维中下标为n的元素的所有值。
#np.random.uniform随机采样，numpy.random.uniform(low,high,size)，输出结果形式为size参数
        if self.use_char:
            scale = np.sqrt(3.0 / self.word_emb_dim)
            data.pretrain_word_embedding[0,:] = np.random.uniform(-scale, scale, [1, self.word_emb_dim])

        self.gaz_embedding = nn.Embedding(data.gaz_alphabet.size(), self.gaz_emb_dim)#生成gaz_embedding，参数为（gaz词汇表大小，gaz的维度）
        self.word_embedding = nn.Embedding(data.word_alphabet.size(), self.word_emb_dim)#生成word_embedding
        if self.use_biword:
            self.biword_embedding = nn.Embedding(data.biword_alphabet.size(), self.biword_emb_dim)#将二元数组转化为embedding，参数为（词表大小，维度）
        ...
        #如果pretrain_gaz_embedding不为空则复制一份，为空的话则随机生成一份
        ...
        char_feature_dim = self.word_emb_dim + 4*self.gaz_emb_dim#一个字符特征的维度大小=单词的维度+词汇集合维度*4,每个字符的所有特征
        if self.use_biword:
            char_feature_dim += self.biword_emb_dim#使用二元词集，可不使用
        ...
        if self.gpu:
            ...
            if self.use_bert:
                self.bert_encoder = self.bert_encoder.cuda()#需要用到gpu时，运行改行代码
    ...
    def get_tags(self,gaz_list, word_inputs, biword_inputs, layer_gaz, gaz_count, gaz_chars, gaz_mask_input, gazchar_mask_input, mask, word_seq_lengths, batch_bert, bert_mask):
        ...
        word_embs = self.word_embedding(word_inputs)#得到word_embedding

        if self.use_biword:
            biword_embs = self.biword_embedding(biword_inputs)
            word_embs = torch.cat([word_embs,biword_embs],dim=-1)#如果使用二元单词，将单词嵌入和二元单词嵌入拼接
