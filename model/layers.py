...
class MUFFmodel(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_layer, dropout, gpu=True):
        ...
        self.cnn_layers = [nn.Conv1d(self.hidden_dim, self.hidden_dim, kernel_size=3, padding=1) for i in range(self.num_layer-1)]#如果层数大于1，则加入循环计算
    ...
    def forward(self, input_feature):
        ...
        input_feature = input_feature.transpose(2,1).contiguous()#进行转置，深拷贝，对实际效果没有影响。（三维矩阵）
